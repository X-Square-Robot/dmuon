...
class TPScatterState(NamedTuple):
    ...
    The state pins TP-owner send shards and receive shards until the scatter
    event is visible.
    ...
    those tensors.  Receive shards are also retained explicitly instead of
    relying solely on ``record_stream`` so the async overlap path has the
    same object lifetime contract as the send path.
    """
