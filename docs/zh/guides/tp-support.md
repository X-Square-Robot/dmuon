...
* `Muon(..., replicate_async=...)` 控制 DP/HSDP post-step publish overlap。
  存在 TP-sharded dedicated 参数时，DMuon 当前会为正确性回退到同步
  post-step publish。
...
当前版本默认不启用 TP async publish。TP 诊断测试仍会覆盖底层
scatter/publish 状态机；面向用户的训练路径会保持同步，直到公开 TP
矩阵覆盖 sync-vs-async parity。
...
* [检查点](checkpoint.md) — dedicated params 的 state-dict 行为
* [通信成本分析](../reference/communication-cost.md) — broadcast / reduce 成本模型
