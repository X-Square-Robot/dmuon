...
# ---------------------------------------------------------------------------
# Tests — fallback bool gating
# ---------------------------------------------------------------------------


def test_sync_fallback_short_circuits_dispatch():
    """When a group is in sync-fallback mode, dispatch+consume should
    happen inline (no PENDING state persists across calls)."""
    # Model the fallback path as dispatch → consume in one call.
    g = _FakeGroup(owned_data="weights")
    sync_fallback = True
    if sync_fallback:
        g.dispatch_async()
        g.consume_wait()   # inline
    assert g._state is _State.IDLE


def test_fallback_trigger_threshold():
    """C.4 fallback: 3 consecutive slow waits flip a flag.  Pure state-
    machine check — no actual timing."""
    SLOW_US = 500.0
    THRESH = 100.0
    CONSEC = 3
    ...
    slow_count = 0
    fallback = False
    for wait_us in [50, 50, 50, 600, 700, 800]:  # first 3 fast, then 3 slow
        if wait_us > THRESH:
            slow_count += 1
        else:
            slow_count = 0
        if slow_count >= CONSEC:
            fallback = True
    assert fallback is True

    slow_count = 0
    fallback = False
    for wait_us in [600, 50, 600, 50, 600]:  # never 3-in-a-row
        if wait_us > THRESH:
            slow_count += 1
        else:
            slow_count = 0
